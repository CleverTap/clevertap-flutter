...
class _MyAppState extends State<MyApp> {
        ...
        padding: const EdgeInsets.all(8.0),
          child: Column(
            ...
            children: <Widget>[
              ...
              RaisedButton(
                  onPressed: () => getProfileProperty(),
                  child: Text('Get Profile Property')
              ),
              RaisedButton(
  ...
  void getProfileProperty(){
    CleverTapPlugin.profileGetProperty("props").then((profileProp) {
      if (profileProp == null) return;
      setState((() {
        print("profile property = "+ profileProp.toString());
      }));
    }).catchError((error) {
      setState(() {
        print("$error");
